...
// FunÃƒÂ§ÃƒÂ£o auxiliar para criar um mÃƒÂ©todo de pagamento simulado
const createPaymentMethod = (name: string, numCard: string, cvv: number, expiryDate: string, type: CardType, cpf: string): Omit<PaymentMethod, 'id'> => {
    return {
        ...
        type,
        cpf,
        clientId: 1, // Assumimos que o ID do cliente ÃƒÂ© 1 para simplicidade
    };
};
...
defineFeature(feature, (test) => {
    ...
    const givenPaymentMethodExistsWithData = (given: DefineStepFunction) =>
      given(/^o método de pagamento "(.*)" está cadastrado com numCard "(.*)", cvv "(.*)", validade "(.*)", tipo "(.*)", cpf "(.*)"$/, async (name, numCard, cvv, validade, tipo, cpf) => {
          const cliente = await createClient();
          clients.push(cliente);
          prismaMock.client.findUnique.mockResolvedValue(cliente);
          const existingPaymentMethod: PaymentMethod = {
    ...
    test('Tentativa de Cadastro de Método de pagamento já cadastrado', ({ given, when, then }) => {
