            ...
            <Box p="50px" position="relative">
                ...
                <Flex flexWrap="wrap" gap="75px" mt="50px">
                    {reservations.filter(reservation => reservation.hotelier_id === Number(data?.id)).sort((a, b) => a.id - b.id).map(reservation => (
                        <Box data-cy={`${replaceSpacesAndRemoveAccents(reservation.name)}`} id={`${replaceSpacesAndRemoveAccents(reservation.name)}`} position="relative" w="290px" h="330px" _hover={{transform: 'translateY(-5px)'}}>
                            ...
                            <Box position="relative" w="290px" h="330px" bg="transparent"  borderRadius="10px" overflow="hidden" color="#191919" cursor="pointer" key={reservation.id} onClick={() => handleReservationClick(reservation)}>
