      ...
      <Box p="50px" position="relative">
        ...
        <Flex flexWrap="wrap" gap="75px" mt="42px">
          {reservations
            ...
            .map((reservation) => (
              <Box
                id={`${replaceSpacesAndRemoveAccents(reservation.name)}`}
                position="relative"
                mb="-3%"
                w="250px"
                h="320px"
              ...
              >
                {reservation.promotion_id && (
                  <Flex
                    ...
                    zIndex="1"
                    style={{ backgroundImage: `url(${morcegoImg})` }}
                  >
                    {" "}
                    <Box transform={"translateY(-60%)"} fontSize="13px">
                      {promotion[reservation.id] !== 0 &&
                        `${promotion[reservation.id]}%`}
                    </Box>
                  </Flex>
                )}
                <Box
                  ...
                  key={reservation.id}
                  onClick={() =>
                    navigate(`/select-reservation/${reservation.id}`)
                  }
                >
                  <Box
                    w="100%"
                    ...
                    style={{
                      backgroundImage: `url(http://localhost:5001${reservation.imageUrl})`,
                    }}
                  ></Box>
                  <Box
                    fontSize="20px"
                  ...
                  <Flex textAlign="start" fontSize="20px" color="#eaeaea">
                    R$ {reservation.new_price.toFixed(0)}{" "}
                    {reservation.promotion_id && (
                      <Box
                        fontSize="15px"
