...
// Function to log in as Hotelier using cy.request
const loginAsHotelier = (username: string, password: string) => {
  return cy
    .request<LoginResponse>({
      method: "POST",
      url: "http://localhost:5001/auth/hotelier/login", // Replace with your actual login endpoint
...
const deleteUserById = (userId: string) => {
  return cy
    .request({
      method: "DELETE",
      url: `http://localhost:5001/hotelier/delete/${userId}`, // Replace with your actual API endpoint
      headers: {
...
Before({ tags: "@deleteUserHotelier" }, function () {
  loginAsHotelier("mateusgaldino", "pesquisarehbom")
    .then(() => {
